...
function connect($new_link=false) {
	$this->link_id=@mysql_connect($this->server,$this->user,$this->pass,$new_link,65536);	// Last parameter to get the recordset as a result from SP call
...
function fetch_all_array($sql) {
	...
	while (($row = $this->fetch_array($query_id, $sql)) != false){
		$out[] = $row;
	}
...
function query_insert($table, $data) {
	...
	foreach($data as $key=>$val) {
		...
		elseif(strtolower($val)=='curdate()') $v.="CURDATE(), ";
		elseif(strtolower($val)=='curtime()') $v.="CURTIME(), ";
